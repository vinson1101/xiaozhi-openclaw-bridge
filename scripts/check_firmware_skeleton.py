...
def main() -> None:
    ...
    assert "listen cancel requested" in main_c and "vb6824_voice_abort_requested = true" in main_c, "middle button must request listening cancel"
    assert "listen submit requested" not in main_c and "set_avatar_dialog(\"SENDING\"" not in main_c, "middle button must not submit while listening"
    ...
    assert "vb6824_voice_submit_pending" not in main_c, "middle listening cancel must not keep the old submit-pending path"
    assert "XOB_BUTTON_LISTEN_COOLDOWN_MS 250" in main_c and "button ignored during cooldown" in main_c, "middle button needs debounce without swallowing normal toggles"
