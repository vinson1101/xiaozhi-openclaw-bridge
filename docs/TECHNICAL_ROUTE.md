...
curl / simulator / firmware
        |
        v
Bridge HTTP JSON API, WebSocket voice sessions
        |
        v
...
Hermes product-demo target:
Bridge-managed persistent Hermes session + progress events
```
...
面向“小元”多轮对话和产品展示时，Bridge 继续是设备侧唯一入口。每个来访者
会话绑定三个标识：`device_session_id -> bridge_session_id -> hermes_session_id`。
Bridge 负责创建、恢复、空闲结束和审计；Hermes 负责同一会话内的对话上下文、
产品知识、Skill 和浏览器任务。不能每一句都重新启动一个无状态的 Hermes CLI
进程。

产品展示所需的最小事件边界：

- Bridge -> Hermes：用户最终转写、取消请求、会话结束。
- Hermes -> Bridge：开始处理、可朗读的文本片段、工具进度、最终结果、审批或错误。
- Bridge -> 板端：`thinking` / `speaking` 状态、分句 TTS、OPUS 音频和 `abort`。

先保持现有 Bridge ASR、TTS、OPUS 和中断链路不变。Hermes 的原生 STT/TTS
只用于主机调试，或在未来作为 Bridge provider 评估；它不是板端接入路径。
...
当前同步 Hermes CLI 路线是第一轮连通性验证，不能作为产品展示的最终模型。
Phase 3B 先实现逻辑上的持久 Hermes 会话和事件转发；设备 WebSocket 是否随之
升级为长生命周期音频通道，应独立演进，不能改变既有的 listening、speaking、
abort 交互 contract。
...
3. Hermes adapter：第一轮 CLI 连通性已完成；当前采用 Hermes 支持的
   `chat --quiet` 持久会话能力，由 Bridge 创建/恢复 Hermes session。事件流
   只在 Hermes 暴露稳定接口后接入，不能靠解析交互式 CLI 输出模拟。
...
5. 可选本地 ASR/TTS：只在 VPS 或 LAN host 资源允许时做，不在 ESP32-C3
   ...
   播放状态和 interrupt 编排。主机本地声卡只服务该主机的调试，不是小元
   面向来访者的设备方案。
...
| Hermes 会话 | Bridge 管理的持久会话 | 支持多轮上下文、展示进度和中断 |
| 主机原生声卡 | 仅调试 | 不能替代板端协议和体验状态机 |
